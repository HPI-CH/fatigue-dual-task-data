...
def test_mean(df,data_type, group_by, norm_dist=True, var='value'):
    ...
    if data_type in ['lac', 'HR']:
        cols = [col for col in df.columns if np.logical_and(data_type in col, 'fatigue' in col)]
    else:
    ...
    if len(cols) == 2:
        if norm_dist:
            ...
            w, p_value_w = stats.wilcoxon(df[cols[0]], df[cols[1]], alternative='two-sided')
            # one-sided Wilcox test
            w1, p_value_w1 = stats.wilcoxon(df[cols[0]], df[cols[1]], alternative='greater')
            w2, p_value_w2 = stats.wilcoxon(df[cols[0]], df[cols[1]], alternative='less')
            print('Wilcoxon test')
            print(f'two-sided, p = {p_value_w}')
            print('H1: ' + cols[0] + ' is greater than ' + cols[1] + ', p = ' + str(p_value_w1))
            print('H1: ' + cols[0] + ' is less than ' + cols[1] + ', p = ' + str(p_value_w2))
