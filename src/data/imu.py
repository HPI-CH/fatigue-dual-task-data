...
class IMU:
    ...
    def __init__(self, data_source, file_type="Physilog", from_json=False):
        if from_json:
            ...
            self.data["timestamp"] = (
                self.data["timestamp"] - self.data.loc[self.data.index[0], "timestamp"]
            )
    ...
    def accel_df(self):
        df = pd.DataFrame(
            self.accel(), columns=["AccX", "AccY", "AccZ"], index=self.time()
        )
    ...
    def crop(self, start_cut, end_cut, inplace=True):
        if inplace:
            self.data = self.data[
                (self.data.index >= start_cut) & (self.data.index < end_cut)
            ]
            self.data.reset_index(drop=True, inplace=True)
        else:
            self_copy = copy.deepcopy(self)
            self_copy.data = self_copy.data[
                (self_copy.data.index >= start_cut) & (self_copy.data.index < end_cut)
            ]
    ...
    def crop_by_time(self, start_cut, end_cut, inplace=True):
        if inplace:
            self.data = self.data[
                (self.data.timestamp >= start_cut) & (self.data.timestamp < end_cut)
            ]
            self.data.reset_index(drop=True, inplace=True)
        else:
            self_copy = copy.deepcopy(self)
            self_copy.data = self_copy.data[
                (self_copy.data.timestamp >= start_cut)
                & (self_copy.data.timestamp < end_cut)
            ]
            self_copy.data.reset_index(drop=True, inplace=True)
            return self_copy
    ...
    def calibrate(self, min_time, max_time):
        calibration_data = self.data[
            (self.data["timestamp"] >= min_time) & (self.data["timestamp"] < max_time)
        ]
