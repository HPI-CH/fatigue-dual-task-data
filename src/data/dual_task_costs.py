...
for run in ["control", "fatigue"]:
    ...
    for sub in sub_list:
        ...
        LR_st_path = os.path.join(
            processed_base_path, "OG_st_" + run, sub, "aggregate_params.csv"
        )

        LR_dt_path = os.path.join(
            processed_base_path, "OG_dt_" + run, sub, "aggregate_params.csv"
        )
    ...
    dt_cost_summary["mean_std"] = (
        "$"
        + dt_cost_summary["mean"].astype(str)
        + " \pm "
        + dt_cost_summary["std"].astype(str)
        + "$"
    )
...
all_dt_cost_summary.index = all_dt_cost_summary.index.str.replace(
    "Cv", "CV", regex=True
)
all_dt_cost_summary.index = all_dt_cost_summary.index.str.replace(
    "Si", "SI", regex=True
