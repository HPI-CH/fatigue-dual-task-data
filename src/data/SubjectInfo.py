...
class SubjectInfo:
    ...
    def __init__(self):
        ...
        if 'st_lac_baseline' not in self.data_df:
            self.data_df['st_lac_baseline'] = self.data_df[['st_lac_baseline1', 'st_lac_baseline2']].mean(axis=1)
        if 'dt_lac_baseline' not in self.data_df:
            self.data_df['dt_lac_baseline'] = self.data_df[['dt_lac_baseline1', 'dt_lac_baseline2']].mean(axis=1)
    ...
    def append_visits_lac(self):
        ...
        @rtype:
        """
        self.plot_df.loc[np.logical_or(
            np.logical_and(self.plot_df['dual_task_visit'] == 1,
                           self.plot_df['condition'].str.contains('dt.*baseline')),
            np.logical_and(self.plot_df['dual_task_visit'] == 2,
                           self.plot_df['condition'].str.contains('st.*baseline'))),
                         'visit'] = '1st Visit Control'
    ...
    def get_plot_df(self, data_type, group_by):
        ...
        if data_type in ['lac', 'HR']:  # if two data points per visit (baseline & fatigue)
            if 'lac' in data_type:
                value_vars_kw = 'lac'
            elif 'HR' in data_type:
                value_vars_kw = 'HR'

            self.plot_df = pd.melt(self.data_df,
                                   id_vars=['sub', 'sex', 'dual_task_visit'],
                                   value_vars=['st_' + value_vars_kw + '_baseline',
    ...
    def get_stats_df(self, data_type, group_by):
        ...
        data_cols[:] = [x for x in data_cols if np.logical_and('baseline1' not in x, 'baseline2' not in x)]
        self.stats_df = self.data_df[data_cols]

        if group_by == 'visit':
            # rename columns, default: st is 1st visit
            self.stats_df.columns = self.stats_df.columns.str.replace('st_', '1st_visit_')
            self.stats_df.columns = self.stats_df.columns.str.replace('dt_', '2nd_visit_')

            # now correct the rows where dt is 1st visit
            mask = self.stats_df['dual_task_visit'] == 1
            if data_type in ['time', 'borg']:
                self.stats_df.loc[mask, ['1st_visit_' + self.col_names[data_type],
                                         '2nd_visit_' + self.col_names[data_type]]] = (
                    self.stats_df.loc[mask, ['2nd_visit_' + self.col_names[data_type],
                                             '1st_visit_' + self.col_names[data_type]]].values)
                print('')
            elif data_type in ['lac', 'HR']:
                self.stats_df.loc[mask, ['1st_visit_' + self.col_names[data_type] + '_baseline',
                    ...
                    self.stats_df.loc[mask, ['2nd_visit_' + self.col_names[data_type] + '_baseline',
                                             '1st_visit_' + self.col_names[data_type] + '_baseline',
                                             '2nd_visit_' + self.col_names[data_type] + '_fatigue',
                                             '1st_visit_' + self.col_names[data_type] + '_fatigue']].values)
    ...
    def boxplot_info(self, group, title, save_fig_path):
        ...
        ax = sns.boxplot(x=group, y='value', data=self.plot_df)  # color=plt.cm.winter_r(100), fliersize=2)  # , #palette='Paired')
    ...
    def main(boxplot=False, statistics=False):
        ...
        if boxplot:
            ...
            # make boxplots
            dat_type = 'borg'  # lac (lactate), HR (heart rate), time (time to fatigue), borg
            grou_by = 'condition'  # visit, condition
            sub_info.get_plot_df(dat_type, grou_by)
            sub_info.get_stats_df(dat_type, grou_by)
            sub_info.boxplot_info(grou_by, 'Lactate Single Task vs. Dual Task', save_fig_path)
