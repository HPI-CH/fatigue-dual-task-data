...
runs = ["OG_st_control", "OG_dt_control", "OG_st_fatigue", "OG_dt_fatigue"]
...
for sub in sub_list:
    for location in ["LF", "SA"]:
        ...
        for cond in ["st", "dt"]:
            ...
            imu_exercise = IMU(
                os.path.join(
                    interim_base_path, f"OG_{cond}_sit_to_stand", sub, f"{location}.csv"
                )
            )
            append_imu_to_df(
            ...
            imu_all = IMU(
                os.path.join(
                    interim_base_path, f"OG_{cond}_all", sub, f"{location}.csv"
                )
            )
...
imu_stats_summary_df.to_csv(
    os.path.join(processed_base_path, "imu_stats_summary.csv"), index=False
)
...
for condition in ["control", "fatigue"]:
    ...
    for sub in sub_list:
        st_path = os.path.join(
            processed_base_path, "OG_st_" + condition, sub, "aggregate_params.csv"
        )

        dt_path = os.path.join(
            processed_base_path, "OG_dt_" + condition, sub, "aggregate_params.csv"
        )
